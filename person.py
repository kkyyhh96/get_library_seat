...
class person(object):
    ...
    def login_page(self):
        # 个人信息
        params = dict(username=str(self.username), password=str(self.password), captcha=self.code)
        ...
        r = requests.post(url=sign_in_url, cookies=self.cookies, params=params, headers=headers, timeout=3)
    ...
    def get_seat(self):
        if self.if_get_seat is False:
            ...
            r = requests.post(url=register_url, cookies=self.cookies, params=params, timeout=2)
